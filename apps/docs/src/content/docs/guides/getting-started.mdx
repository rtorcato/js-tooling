...
`fix` defaults to **No** when overwriting existing files — your customisations are preserved unless you explicitly opt in (or pass `--yes`). See [CLI reference](/js-tooling/guides/cli/#fix-target) for full details.
